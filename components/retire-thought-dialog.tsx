...
import { retireThought } from "@/lib/thoughts"
import { cn } from "@/lib/utils"

interface RetireThoughtDialogProps {
  thought: Thought
  isOpen: boolean
  onClose: () => void
  onRetired: () => void
...
export function RetireThoughtDialog({ thought, isOpen, onClose, onRetired }: RetireThoughtDialogProps) {
  ...
  const handleRetire = async (mode: "release" | "archive") => {
    ...
    const result = await retireThought(thought.id, mode)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Retire Thought</DialogTitle>
          <DialogDescription>
            Choose how you want to retire this thought.
          </DialogDescription>
        </DialogHeader>
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-muted-foreground">
                  Keep the thought for reference, but remove from active rotation
                </p>
              </div>
              {isSubmitting && (
